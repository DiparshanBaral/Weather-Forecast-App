...
window.addEventListener("laod",fetchDefault());


async function fetchDefault(){
    const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=poole&appid=cf36667e803654aced1e658be59cee73&units=metric`);
...
async function fetchData(){
    ...
    const response = await fetch(`https://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=cf36667e803654aced1e658be59cee73&units=metric`);
    const datas = await response.json();
    console.log(datas)
    if (datas.message == "Nothing to geocode"){
        document.getElementById("error_message").innerHTML = "Input cannot be empty." // Display error message if input is empty.
